...
// Fetch form and questions data from Firebase
db.collection('forms').doc(docId).get().then((doc) => {
    if (doc.exists) {
        const formData = doc.data();
        //console.log(JSON.stringify(formData))
        window.form = JSON.parse(formData.form)
        window.questions = JSON.parse(formData.questions)
        createForm(form)
...
function submitForm(redirect) {
    ...
    // Assuming 'form' is a variable containing the form identifier
    // Replace this variable with the actual value or fetch it as needed
    for (var i in questions) {
    ...
    var formData = JSON.stringify(formResponses);

    // Assuming you've initialized Firebase and have a reference to the database
    const db = firebase.firestore();

    // Path to the collection where the responses will be stored
    const responsesRef = db.collection('forms').doc(docId).collection('responses');

    responsesRef.add({
            formData
        })
        .then((docRef) => {
            console.log("Response submitted with ID: ", docRef.id);
